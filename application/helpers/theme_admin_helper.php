...
if (!function_exists('theme_url')) {
	function theme_url($url=null, $ver=false) {
		$version = null;
		if ($ver && $ver === true) {
			$file = _ci()->theme_admin->config('path') . _ci()->theme_admin->config('theme_admin') . '/'. ltrim($url, '/');
			if (file_exists($file)) {
		...
		return base_url('/themes/'._ci()->theme_admin->config('theme_admin').'/'.ltrim($url, '/') . $version);
	}
}
