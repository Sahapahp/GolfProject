...
class Theme_admin {
	...
	public function __construct() {
		...
		$this->config = config_item('theme_admin');
	...
	public function set_theme($theme = 'default') {
		$this->config['theme_admin'] = $theme;
		
		$functions = $this->config('path').$this->config('theme_admin').'/functions.php';
    	if (file_exists($functions)) {
    		include_once($functions);
    	}

        $this->template_locations = array($this->config('path') . $this->config('theme_admin') . '/views/modules/' . $this->module .'/',
                                          $this->config('path') . $this->config('theme_admin') . '/views/',
	...
	public function set_layout($layout = 'index') {
		$path = $this->config('path').$this->config('theme_admin').'/'.$layout.'.php';
		if (!file_exists($path)) $layout = 'index';
		$this->config['layout'] = $layout;
	}
	
	public function template_part($view=null, $return=false) {
		$path = $this->config('path') . $this->config('theme_admin') . '/' . $view . EXT;
		if (file_exists($path)) {
	...
	public function element($element=null, $data=null) {
		$path = $this->config('path') . $this->config('theme_admin') . '/views/elements/' . $element;
	...
	public function render($view, $data=null) {
		...
		$theme = $this->config('path') . $this->config('theme_admin') . '/' . $this->config('layout');
